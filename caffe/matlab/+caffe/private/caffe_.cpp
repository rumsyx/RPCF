...
static void set_resolution_index(MEX_ARGS) {
   mxCHECK( mxIsStruct(prhs[0]) && mxIsSingle(prhs[1]),
         "Usage: caffe_('blob_set_data', hBlob, new_data)");
      
  float resolution_index = static_cast<int>(mxGetScalar(prhs[1]));
  Layer<float>::resolution_index.resize(1);
  Layer<float>::resolution_index[0].reset(new Blob<float>());
  Layer<float>::resolution_index[0]->Reshape(1,1,1,1);
  float *data;
 data=Layer<float>::resolution_index[0]->mutable_cpu_data();
  data[0]=resolution_index;
 }

static void input_App(MEX_ARGS) {
   mxCHECK( mxIsStruct(prhs[0]) && mxIsSingle(prhs[1]),
         "Usage: caffe_('blob_set_data', hBlob, new_data)");
  const int* dims = static_cast<const int*>(mxGetDimensions(prhs[1])); 
  const int* dims1 = static_cast<const int*>(mxGetDimensions(prhs[2])); 
  const int* dims2 = static_cast<const int*>(mxGetDimensions(prhs[4])); 
  const int* dims3 = static_cast<const int*>(mxGetDimensions(prhs[5]));
  const int* dims4 = static_cast<const int*>(mxGetDimensions(prhs[6])); 
  const int* dims5 = static_cast<const int*>(mxGetDimensions(prhs[7]));   
  const int* dims6 = static_cast<const int*>(mxGetDimensions(prhs[8])); 
  const int* dims7 = static_cast<const int*>(mxGetDimensions(prhs[9]));  


  int blob_id = static_cast<int>(mxGetScalar(prhs[3]));

  Blob<float>* tmp6=new Blob<float>();
  tmp6->Reshape(1,1,dims4[1],dims4[0]);
  Layer<float>::ATAW_positive_index[blob_id].reset(new Blob<float>());
  Layer<float>::ATAW_positive_index[blob_id]->Reshape(1,1,dims4[1],dims4[0]);  
  mx_mat_to_blob(prhs[6],tmp6,DATA);
  caffe_copy(tmp6->count(),tmp6->mutable_gpu_data(),Layer<float>::ATAW_positive_index[blob_id]->mutable_gpu_data());


  Blob<float>* tmp7=new Blob<float>();
  tmp7->Reshape(1,1,dims5[1],dims5[0]);
  Layer<float>::ATAW_negative_index[blob_id].reset(new Blob<float>());
  Layer<float>::ATAW_negative_index[blob_id]->Reshape(1,1,dims5[1],dims5[0]);
  mx_mat_to_blob(prhs[7],tmp7,DATA);
  caffe_copy(tmp7->count(),tmp7->mutable_gpu_data(),Layer<float>::ATAW_negative_index[blob_id]->mutable_gpu_data());


  Blob<float>* tmp8=new Blob<float>();
  tmp8->Reshape(1,1,dims6[1],dims6[0]);
  Layer<float>::ATAW_positive_index1[blob_id].reset(new Blob<float>());
  Layer<float>::ATAW_positive_index1[blob_id]->Reshape(1,1,dims6[1],dims6[0]);  
  mx_mat_to_blob(prhs[8],tmp8,DATA);
  caffe_copy(tmp8->count(),tmp8->mutable_gpu_data(),Layer<float>::ATAW_positive_index1[blob_id]->mutable_gpu_data());


  Blob<float>* tmp9=new Blob<float>();
  tmp9->Reshape(1,1,dims7[1],dims7[0]);
  Layer<float>::ATAW_negative_index1[blob_id].reset(new Blob<float>());
  Layer<float>::ATAW_negative_index1[blob_id]->Reshape(1,1,dims7[1],dims7[0]);
  mx_mat_to_blob(prhs[9],tmp9,DATA);
  caffe_copy(tmp9->count(),tmp9->mutable_gpu_data(),Layer<float>::ATAW_negative_index1[blob_id]->mutable_gpu_data());
  ...
  Blob<float>* tmp3=new Blob<float>(); //存储fft的虚部
  tmp3->Reshape(1,1,dims3[1],dims3[0]);
  mx_mat_to_blob(prhs[4],tmp2, DATA);
  mx_mat_to_blob(prhs[5],tmp3, DATA); 

  
  Layer<float>::Ap[blob_id].reset(new Blob<float>());
  Layer<float>::Ap[blob_id]->Reshape(1,1,dims[1],dims[0]);
 
  Layer<float>::AW[blob_id].reset(new Blob<float>());
    Layer<float>::AW[blob_id]->Reshape(1,1,1,Layer<float>::Ap[blob_id]->height()*Layer<float>::H_masked[blob_id]->channels());

   Layer<float>::zelta[blob_id].reset(new Blob<float>());
   Layer<float>::zelta[blob_id]->Reshape(1,1,1,Layer<float>::Ap[blob_id]->height()*Layer<float>::H_masked[blob_id]->channels());


  Layer<float>::Ap1[blob_id].reset(new Blob<float>());
  Layer<float>::Ap1[blob_id]->Reshape(1,1,dims1[1],dims1[0]); 
 
  Layer<float>::AW1[blob_id].reset(new Blob<float>());
  Layer<float>::AW1[blob_id]->Reshape(1,1,1,Layer<float>::Ap1[blob_id]->height()*Layer<float>::H_masked[blob_id]->channels());

  Layer<float>::App[blob_id].reset(new Blob<float>());
  Layer<float>::App[blob_id]->Reshape(1,1,dims2[1],dims2[0]);

  Layer<float>::App1[blob_id].reset(new Blob<float>());
  Layer<float>::App1[blob_id]->Reshape(1,1,dims3[1],dims3[0]);
  
  caffe_copy(tmp2->count(),tmp2->mutable_gpu_data(),Layer<float>::App[blob_id]->mutable_gpu_data());
  caffe_copy(tmp3->count(),tmp3->mutable_gpu_data(),Layer<float>::App1[blob_id]->mutable_gpu_data());

  caffe_copy(tmp->count(),tmp->mutable_gpu_data(),Layer<float>::Ap[blob_id]->mutable_gpu_data());
  caffe_copy(tmp1->count(),tmp1->mutable_gpu_data(),Layer<float>::Ap1[blob_id]->mutable_gpu_data());
  delete tmp;
...
static void inialize_blobs(MEX_ARGS) {
    ...
    Layer<float>::eta.resize(1);
    Layer<float>::eta[0].reset(new Blob<float>());
    Layer<float>::eta[0]->Reshape(1,1,1,1);

    Layer<float>::mu.resize(1);
    Layer<float>::mu[0].reset(new Blob<float>());
    Layer<float>::mu[0]->Reshape(1,1,1,1); 

    if(net_index[0]==1)
    {

      Layer<float>::zelta.resize(5);
   

      Layer<float>::rho.resize(1);
      Layer<float>::rho[0].reset(new Blob<float>());
      Layer<float>::rho[0]->Reshape(1,1,1,1);

      Layer<float>::inner_product_result.resize(1);
      Layer<float>::inner_product_result[0].reset(new Blob<float>());
      Layer<float>::inner_product_result[0]->Reshape(1,1,1,1);

      Layer<float>::middle_line.resize(5);
      Layer<float>::middle_line[0].reset(new Blob<float>());
      Layer<float>::middle_line[0]->Reshape(1,1,height[0],1);


      Layer<float>::ATAW_positive_index.resize(5);
      Layer<float>::ATAW_negative_index.resize(5);
      Layer<float>::ATAW_positive_index1.resize(5);
      Layer<float>::ATAW_negative_index1.resize(5);  

      Layer<float>::App.resize(5);
      Layer<float>::App1.resize(5);

      Layer<float>::AW.resize(5);
      Layer<float>::AW1.resize(5);  

      Layer<float>::H_transpose.resize(5);
      Layer<float>::H_transpose[0].reset(new Blob<float>());
      Layer<float>::H_transpose[0]->Reshape(1,channel[0],width[0],height[0]);

      Layer<float>::Ap.resize(5);
      Layer<float>::Ap1.resize(5);
      ...
      Layer<float>::H_masked.resize(5);
      Layer<float>::H_masked[0].reset(new Blob<float>());
      Layer<float>::H_masked[0]->Reshape(1,channel[0],height[0],width[0]);   

      Layer<float>::H_reged.resize(5);
      Layer<float>::H_reged[0].reset(new Blob<float>());
      Layer<float>::H_reged[0]->Reshape(1,channel[0],height[0],width[0]);   

      Layer<float>::ATAW_MC.resize(5);
      Layer<float>::ATAW_MC[0].reset(new Blob<float>());
      Layer<float>::ATAW_MC[0]->Reshape(1,channel[0],height[0],width[0]); 
    ...
    {
      Layer<float>::middle_line[1].reset(new Blob<float>());
      Layer<float>::middle_line[1]->Reshape(1,1,height[0],1);

      Layer<float>::H_transpose[1].reset(new Blob<float>());
      Layer<float>::H_transpose[1]->Reshape(1,channel[0],width[0],height[0]);
      ...
      Layer<float>::H_masked[1].reset(new Blob<float>());
      Layer<float>::H_masked[1]->Reshape(1,channel[0],height[0],width[0]);  

      Layer<float>::H_reged[1].reset(new Blob<float>());
      Layer<float>::H_reged[1]->Reshape(1,channel[0],height[0],width[0]);

      Layer<float>::ATAW_MC[1].reset(new Blob<float>());
      Layer<float>::ATAW_MC[1]->Reshape(1,channel[0],height[0],width[0]); 
    ...
    {
      Layer<float>::middle_line[2].reset(new Blob<float>());
      Layer<float>::middle_line[2]->Reshape(1,1,height[0],1);

      Layer<float>::H_transpose[2].reset(new Blob<float>());
      Layer<float>::H_transpose[2]->Reshape(1,channel[0],width[0],height[0]);
      ...
      Layer<float>::H_masked[2].reset(new Blob<float>());
      Layer<float>::H_masked[2]->Reshape(1,channel[0],height[0],width[0]);  

      Layer<float>::H_reged[2].reset(new Blob<float>());
      Layer<float>::H_reged[2]->Reshape(1,channel[0],height[0],width[0]);

      Layer<float>::ATAW_MC[2].reset(new Blob<float>());
      Layer<float>::ATAW_MC[2]->Reshape(1,channel[0],height[0],width[0]); 
    ...
    {
      Layer<float>::middle_line[3].reset(new Blob<float>());
      Layer<float>::middle_line[3]->Reshape(1,1,height[0],1); 

      Layer<float>::H_transpose[3].reset(new Blob<float>());
      Layer<float>::H_transpose[3]->Reshape(1,channel[0],width[0],height[0]);
      ...
      Layer<float>::H_masked[3].reset(new Blob<float>());
      Layer<float>::H_masked[3]->Reshape(1,channel[0],height[0],width[0]);  

      Layer<float>::H_reged[3].reset(new Blob<float>());
      Layer<float>::H_reged[3]->Reshape(1,channel[0],height[0],width[0]);

      Layer<float>::ATAW_MC[3].reset(new Blob<float>());
      Layer<float>::ATAW_MC[3]->Reshape(1,channel[0],height[0],width[0]); 
    ...
    {
      Layer<float>::middle_line[4].reset(new Blob<float>());
      Layer<float>::middle_line[4]->Reshape(1,1,height[0],1);

      Layer<float>::H_transpose[4].reset(new Blob<float>());
      Layer<float>::H_transpose[4]->Reshape(1,channel[0],width[0],height[0]);  
      ...
      Layer<float>::H_masked[4].reset(new Blob<float>());
      Layer<float>::H_masked[4]->Reshape(1,channel[0],height[0],width[0]);  

      Layer<float>::H_reged[4].reset(new Blob<float>());
      Layer<float>::H_reged[4]->Reshape(1,channel[0],height[0],width[0]);

      Layer<float>::ATAW_MC[4].reset(new Blob<float>());
      Layer<float>::ATAW_MC[4]->Reshape(1,channel[0],height[0],width[0]); 
...
static void set_pcg_paramters(MEX_ARGS) {
   mxCHECK( mxIsStruct(prhs[0]) && mxIsSingle(prhs[1]),
         "Usage: caffe_('blob_set_data', hBlob, new_data)");
  ...
  // 首先从matlab中输入hf的实部和虚部 
    //
  const  mxArray* mx_mat1=prhs[1];
  const float* rho = reinterpret_cast<const float*>(mxGetData(mx_mat1));
  float* data=Layer<float>::rho[0]->mutable_cpu_data();
  data[0]=rho[0];
...
static void get_inner_product(MEX_ARGS) {

  Blob<float>* tmp=new Blob<float>(); 
  tmp->Reshape(1,1,1,1);
  
  caffe_copy(tmp->count(),Layer<float>::inner_product_result[0]->mutable_gpu_data(),tmp->mutable_gpu_data()); 
...
static void input_yf(MEX_ARGS) { 
  ...
  const int* dims3 = static_cast<const int*>(mxGetDimensions(prhs[4]));
  ...
  tmp3->Reshape(1,1,dims3[1],dims3[0]);
  mx_mat_to_blob(prhs[4],tmp3,DATA);
  ...
  Layer<float>::first_layer_yf_imag[0]->Reshape(1,1,dims1[1],dims1[0]);

  Layer<float>::first_layer_yf_real1.resize(1);
  Layer<float>::first_layer_yf_real1[0].reset(new Blob<float>());
  Layer<float>::first_layer_yf_real1[0]->Reshape(1,1,dims2[1],dims2[0]);
  Layer<float>::first_layer_yf_imag1.resize(1);
  Layer<float>::first_layer_yf_imag1[0].reset(new Blob<float>());
  Layer<float>::first_layer_yf_imag1[0]->Reshape(1,1,dims3[1],dims3[0]); 
  ...
  caffe_copy(tmp2->count(),tmp2->mutable_gpu_data(),Layer<float>::first_layer_yf_real1[0]->mutable_gpu_data()); 
  caffe_copy(tmp3->count(),tmp3->mutable_gpu_data(),Layer<float>::first_layer_yf_imag1[0]->mutable_gpu_data());  
 delete tmp;
 delete tmp1;
 delete tmp2;
 delete tmp3;
...
static void set_reg_window(MEX_ARGS) { 
  ...
  const int* dims8 = static_cast<const int*>(mxGetDimensions(prhs[8]));
  ...
  Layer<float>::reg_window.resize(2);
  Layer<float>::reg_window[0].reset(new Blob<float>());
  Layer<float>::reg_window[0]->Reshape(1,1,dims5[1],dims5[0]);
  Layer<float>::reg_window[1].reset(new Blob<float>());
  Layer<float>::reg_window[1]->Reshape(1,1,dims6[1],dims6[0]); 
  ...
  tmp7->Reshape(1,1,dims7[1],dims7[0]);
  tmp8->Reshape(1,1,dims8[1],dims8[0]);      
  ...
  caffe_copy(tmp7->count(),tmp7->mutable_gpu_data(),Layer<float>::reg_window[0]->mutable_gpu_data()); 
  caffe_copy(tmp8->count(),tmp8->mutable_gpu_data(),Layer<float>::reg_window[1]->mutable_gpu_data()); 
  

  delete tmp1; delete tmp2; delete tmp3; delete tmp4; delete tmp5; delete tmp6; delete tmp7; delete tmp8;
...
static void set_factor(MEX_ARGS) {
 
    const  mxArray* mx_mat1=prhs[1];
    const float* factor = reinterpret_cast<const float*>(mxGetData(mx_mat1));
    
    Layer<float>::factor.resize(1);
    Layer<float>::factor[0].reset(new Blob<float>());
    Layer<float>::factor[0]->Reshape(1,1,1,1);
    
    float* data=Layer<float>::factor[0]->mutable_cpu_data();
    data[0]=factor[0];
...
static void get_H_masked_4(MEX_ARGS) {

  Blob<float>* tmp1=new Blob<float>();
  tmp1->Reshape(1,Layer<float>::H_masked[0]->channels(),Layer<float>::H_masked[0]->height(),Layer<float>::H_masked[0]->width());
   
  Blob<float>* tmp2=new Blob<float>();
  tmp2->Reshape(1,Layer<float>::H_masked[1]->channels(),Layer<float>::H_masked[1]->height(),Layer<float>::H_masked[1]->width()); 
  
  Blob<float>* tmp3=new Blob<float>();
  tmp3->Reshape(1,Layer<float>::H_masked[2]->channels(),Layer<float>::H_masked[2]->height(),Layer<float>::H_masked[2]->width()); 

  Blob<float>* tmp4=new Blob<float>();
  tmp4->Reshape(1,Layer<float>::H_masked[3]->channels(),Layer<float>::H_masked[3]->height(),Layer<float>::H_masked[3]->width()); 

  caffe_copy(tmp1->count(),Layer<float>::H_masked[0]->mutable_gpu_data(),tmp1->mutable_gpu_data());
  caffe_copy(tmp2->count(),Layer<float>::H_masked[1]->mutable_gpu_data(),tmp2->mutable_gpu_data());
  caffe_copy(tmp3->count(),Layer<float>::H_masked[2]->mutable_gpu_data(),tmp3->mutable_gpu_data());   
  caffe_copy(tmp4->count(),Layer<float>::H_masked[3]->mutable_gpu_data(),tmp4->mutable_gpu_data());

  plhs[0] = blob_to_mx_mat(tmp1, DATA);
  plhs[1] = blob_to_mx_mat(tmp2, DATA);
  plhs[2] = blob_to_mx_mat(tmp3, DATA); 
  plhs[3] = blob_to_mx_mat(tmp4, DATA);   

  delete tmp1; delete tmp2; delete tmp3; delete tmp4;
}

static void get_H_masked_5(MEX_ARGS) {
  ...
  Blob<float>* tmp5=new Blob<float>();
  tmp5->Reshape(1,Layer<float>::H_masked[4]->channels(),Layer<float>::H_masked[4]->height(),Layer<float>::H_masked[4]->width());   

  caffe_copy(tmp1->count(),Layer<float>::H_masked[0]->mutable_gpu_data(),tmp1->mutable_gpu_data());
  caffe_copy(tmp2->count(),Layer<float>::H_masked[1]->mutable_gpu_data(),tmp2->mutable_gpu_data());
  caffe_copy(tmp3->count(),Layer<float>::H_masked[2]->mutable_gpu_data(),tmp3->mutable_gpu_data());   
  caffe_copy(tmp4->count(),Layer<float>::H_masked[3]->mutable_gpu_data(),tmp4->mutable_gpu_data());
  caffe_copy(tmp5->count(),Layer<float>::H_masked[4]->mutable_gpu_data(),tmp5->mutable_gpu_data());  
  ...
  plhs[2] = blob_to_mx_mat(tmp3, DATA); 
  plhs[3] = blob_to_mx_mat(tmp4, DATA);
  plhs[4] = blob_to_mx_mat(tmp5, DATA);     

  delete tmp1; delete tmp2; delete tmp3; delete tmp4; delete tmp5;
}

static void set_parameters(MEX_ARGS) {

  Blob<float>* tmp1=new Blob<float>();
  tmp1->Reshape(1,1,1,1);
   
  Blob<float>* tmp2=new Blob<float>();
  tmp2->Reshape(1,1,1,1); 

  mx_mat_to_blob(prhs[1],tmp1,DATA);
  mx_mat_to_blob(prhs[2],tmp2,DATA);  

  caffe_copy(tmp1->count(),tmp1->mutable_gpu_data(),Layer<float>::mu[0]->mutable_gpu_data());
  caffe_copy(tmp2->count(),tmp2->mutable_gpu_data(),Layer<float>::eta[0]->mutable_gpu_data()); 
 
  delete tmp1; delete tmp2;

}

static void set_zelta_4(MEX_ARGS) {
  ...
  caffe_copy(tmp1->count(),tmp1->mutable_gpu_data(),Layer<float>::zelta[0]->mutable_gpu_data());
  caffe_copy(tmp2->count(),tmp2->mutable_gpu_data(),Layer<float>::zelta[1]->mutable_gpu_data());
  caffe_copy(tmp3->count(),tmp3->mutable_gpu_data(),Layer<float>::zelta[2]->mutable_gpu_data());   
  caffe_copy(tmp4->count(),tmp4->mutable_gpu_data(),Layer<float>::zelta[3]->mutable_gpu_data()); 
 
  delete tmp1; delete tmp2; delete tmp3; delete tmp4;

}

static void set_zelta_5(MEX_ARGS) {
  const int* dims1 = static_cast<const int*>(mxGetDimensions(prhs[1]));
  const int* dims2 = static_cast<const int*>(mxGetDimensions(prhs[2]));
  const int* dims3 = static_cast<const int*>(mxGetDimensions(prhs[3])); 
  const int* dims4 = static_cast<const int*>(mxGetDimensions(prhs[4]));   
  ...
  caffe_copy(tmp3->count(),tmp3->mutable_gpu_data(),Layer<float>::zelta[2]->mutable_gpu_data());   
  caffe_copy(tmp4->count(),tmp4->mutable_gpu_data(),Layer<float>::zelta[3]->mutable_gpu_data()); 
  caffe_copy(tmp5->count(),tmp5->mutable_gpu_data(),Layer<float>::zelta[4]->mutable_gpu_data()); 
...
static void set_ATAW_MC_4(MEX_ARGS) {
  ...
  caffe_copy(tmp1->count(),tmp1->mutable_gpu_data(),Layer<float>::ATAW_MC[0]->mutable_gpu_data());
  caffe_copy(tmp2->count(),tmp2->mutable_gpu_data(),Layer<float>::ATAW_MC[1]->mutable_gpu_data());
  caffe_copy(tmp3->count(),tmp3->mutable_gpu_data(),Layer<float>::ATAW_MC[2]->mutable_gpu_data());   
  caffe_copy(tmp4->count(),tmp4->mutable_gpu_data(),Layer<float>::ATAW_MC[3]->mutable_gpu_data()); 
 
  delete tmp1; delete tmp2; delete tmp3; delete tmp4;

}

static void set_ATAW_MC_5(MEX_ARGS) {
  ...
  caffe_copy(tmp3->count(),tmp3->mutable_gpu_data(),Layer<float>::ATAW_MC[2]->mutable_gpu_data());   
  caffe_copy(tmp4->count(),tmp4->mutable_gpu_data(),Layer<float>::ATAW_MC[3]->mutable_gpu_data());
  caffe_copy(tmp5->count(),tmp5->mutable_gpu_data(),Layer<float>::ATAW_MC[4]->mutable_gpu_data());   
 
  delete tmp1; delete tmp2; delete tmp3; delete tmp4; delete tmp5;

}

static void set_feature_num(MEX_ARGS){
  const  mxArray* mx_mat1=prhs[1];
  const float* input_data = reinterpret_cast<const float*>(mxGetData(mx_mat1));     

  float* feature_num=Layer<float>::feature_num[0]->mutable_cpu_data();
  feature_num[0]=input_data[0];
...
static void get_xf_yf(MEX_ARGS) {

    int height=Layer<float>::matlab_hf_real[0]->height();
    int width=Layer<float>::matlab_hf_real[0]->width();



 Blob<float>* tmp=new Blob<float>();
 tmp->Reshape(1,3,height,width);
    caffe_copy(tmp->count(),Layer<float>::matlab_hf_real[0]->mutable_gpu_data(),tmp->mutable_gpu_data());
plhs[0] = blob_to_mx_mat(tmp, DATA);

Blob<float>* tmp1=new Blob<float>();
 tmp1->Reshape(1,3,height,width);
    caffe_copy(tmp1->count(),Layer<float>::first_layer_hf_real[0]->mutable_gpu_data(),tmp1->mutable_gpu_data());
plhs[1] = blob_to_mx_mat(tmp1, DATA);
...
static void im2col(MEX_ARGS) {
    const int* dims = static_cast<const int*>(mxGetDimensions(prhs[1])); 
    Blob<float> im_data(1, dims[2], dims[1], dims[0]);
    ...
    int col_dim=kernel_h*kernel_w*dims[2];
...
static handler_registry handlers[] = {
  ...
  { "fcn2cnn",              fcn2cnn              },
  { "snapshot",             snapshot             },
  { "get_H_masked_4",       get_H_masked_4       },
  { "get_H_masked_5",       get_H_masked_5       },
  { "set_ATAW_MC_4",        set_ATAW_MC_4        },
  { "set_ATAW_MC_5",        set_ATAW_MC_5        },
  { "set_parameters",       set_parameters       },
  { "input_App",            input_App            },
  { "set_resolution_index", set_resolution_index },
  { "get_xf_yf",            get_xf_yf            },
  { "get_inner_product",    get_inner_product    },
  { "set_pcg_paramters",    set_pcg_paramters    },
  { "set_feature_num",      set_feature_num      },
  { "set_zelta_4",          set_zelta_4          },
  { "set_zelta_5",          set_zelta_5          },
