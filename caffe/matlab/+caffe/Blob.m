...
%       data = self.check_and_preprocess_data(data);
      ...
      function set_resolution_index(self, resolution_index)
%       data = self.check_and_preprocess_data(data);
      caffe_('set_resolution_index', self.hBlob_self, single(resolution_index));
      end
      
       function set_feature_num(self, feature_num)
%       data = self.check_and_preprocess_data(data);
      caffe_('set_feature_num', self.hBlob_self, single(feature_num));
      end
...
%       data = self.check_and_preprocess_data(data);
      caffe_('my_set_data_a1', self.hBlob_self, single(data),single(data1));
        ...
        function input_yf(self, data,data1,data2,data3)
%       data = self.check_and_preprocess_data(data);
      caffe_('input_yf', self.hBlob_self, single(data),single(data1),single(data2),single(data3));
      end
...
function set_reg_window(self, fftshift_mask1,fftshift_mask2,ifftshift_mask1,ifftshift_mask2,binary_mask1,binary_mask2,reg_window1, reg_window2)
%       data = self.check_and_preprocess_data(data);
          caffe_('set_reg_window', self.hBlob_self, single(fftshift_mask1),single(fftshift_mask2),single(ifftshift_mask1),single(ifftshift_mask2),...
              single(binary_mask1),single(binary_mask2),single(reg_window1), single(reg_window2));
end

function set_factor(self, factor)
caffe_('set_factor', self.hBlob_self, single(factor));
end
...
function set_H(self, H1,H2,H3,H4,H5,frag_num)
%       data = self.check_and_preprocess_data(data);
          caffe_('set_H', self.hBlob_self, single(H1), single(H2),  single(H3), single(H4), single(H5),single(frag_num));
end
...
function [H_masked1,H_masked2, H_masked3, H_masked4]=get_H_masked_4(self)
%       data = self.check_and_preprocess_data(data);
          [H_masked1,H_masked2, H_masked3, H_masked4]=caffe_('get_H_masked_4', self.hBlob_self);
end

function [data1,data2]=get_xf_yf(self)
%       data = self.check_and_preprocess_data(data);
          [data1,data2]=caffe_('get_xf_yf', self.hBlob_self);
end

function [data1]=get_inner_product(self)
          [data1]=caffe_('get_inner_product', self.hBlob_self);
end

function [H_masked1,H_masked2, H_masked3, H_masked4,H_masked5]=get_H_masked_5(self)
%       data = self.check_and_preprocess_data(data);
          [H_masked1,H_masked2, H_masked3, H_masked4,H_masked5]=caffe_('get_H_masked_5', self.hBlob_self);
end

function set_ATAW_MC_4(self, ATAW_MC1, ATAW_MC2,ATAW_MC3,ATAW_MC4)
%       data = self.check_and_preprocess_data(data);
          caffe_('set_ATAW_MC_4', self.hBlob_self, single(ATAW_MC1), single(ATAW_MC2),single(ATAW_MC3),single(ATAW_MC4));
end

function set_ATAW_MC_5(self, ATAW_MC1, ATAW_MC2,ATAW_MC3,ATAW_MC4,ATAW_MC5)
%       data = self.check_and_preprocess_data(data);
          caffe_('set_ATAW_MC_5', self.hBlob_self, single(ATAW_MC1), single(ATAW_MC2),single(ATAW_MC3),single(ATAW_MC4),single(ATAW_MC5));
end

function set_parameters(self, mu, eta)
%       data = self.check_and_preprocess_data(data);
          caffe_('set_parameters', self.hBlob_self, single(mu), single(eta));
end

function input_App(self, data1, data2, block_id,App, App1, ATAW_positive_index, ATAW_negative_index, ...
        ATAW_positive_index1, ATAW_negative_index1)
%       data = self.check_and_preprocess_data(data);
          caffe_('input_App', self.hBlob_self, single(data1), single(data2), single(block_id),single(App), single(App1),...
          single(ATAW_positive_index), single(ATAW_negative_index), single(ATAW_positive_index1), single(ATAW_negative_index1));
end

function set_zelta_4(self, data1, data2, data3, data4)
%       data = self.check_and_preprocess_data(data);
          caffe_('set_zelta_4', self.hBlob_self, single(data1), single(data2), single(data3),single(data4));
end

function set_zelta_5(self, data1, data2, data3, data4,data5)
%       data = self.check_and_preprocess_data(data);
          caffe_('set_zelta_4', self.hBlob_self, single(data1), single(data2), single(data3),single(data4),single(data5));
end
