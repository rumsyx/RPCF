...
for block_id=1:num_feature_blocks
     ...
     tmp=real(ifft2(tmp));
          ...
          if block_id~=3
          ATXi1{block_id}=1*compute_ATXi(Ap1{block_id},App1{block_id}, tmp,Xi1{block_id},filter_sz(block_id,:));
          else
              ATXi1{block_id}=1*compute_ATXi(Ap{block_id},App{block_id}, tmp,Xi1{block_id},filter_sz(block_id,:));
          end
          tmp=tmp-ATXi1{block_id};
          tmp=fft2(tmp);
